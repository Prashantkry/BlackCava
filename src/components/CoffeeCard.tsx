...
const CoffeeCard: React.FC<CoffeeCardProps> = ({ product }) => {
  ...
  const handleAddToCart = () => {
    if (quantity === 0) {
      dispatch(removeFromCart({ productId: product.productId, size: selectedSize, quantity: 0 }));
      toast.success(`${product.name} with size ${selectedSize} removed from cart`,{ autoClose: 1500 });
    } else {
      dispatch(addToCart({ productId: product.productId, size: selectedSize, quantity: quantity }));
