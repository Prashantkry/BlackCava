...
const FavoriteCoffeeCard: React.FC<FavoriteCoffeeCardProps> = ({ coffee }) => {
    ...
    const [selectedSize, setSelectedSize] = useState<string>(coffee.sizes[1].size);
    ...
    const cartItem = cart.find((item) => item.productId === coffee.productId && item.size === selectedSize);
    ...
    const handleSizeChange = (size: string) => {
        ...
        const newCartItem = cart.find((item) => item.productId === coffee.productId && item.size === size);
        if (newCartItem) {
    ...
    return (
        <div className="flex flex-col relative sm:flex-row bg-gray-800 rounded-xl shadow-lg overflow-hidden mb-6 w-full h-60 sm:h-72" ref={cardRef}>
            <div className="relative w-full sm:w-1/3 h-full">
                <Image src={coffeeDummyImage} alt={coffee.name} layout="fill" objectFit="cover" />
            </div>
            <div className="flex flex-col gap-3 p-4 w-full sm:w-2/3">
                ...
                <div>
                    <span className="text-yellow-500 font-bold">
                        Price: ${coffee.sizes.find(size => size.size === selectedSize)?.price}
                    </span>
                    <div className="flex justify-between items-center mt-2 w-full gap-2">
                        <div className="flex space-x-2">
                            {coffee.sizes.map((size) => (
                                <button
                                    key={size.size}
                                    className={`w-10 h-8 flex items-center justify-center px-2 py-1 rounded ${selectedSize === size.size ? 'bg-yellow-600' : 'bg-gray-600'}`}
                ...
                {showQuantityInput && (
                    <div className="flex items-center mt-2">
                        <label htmlFor="quantity" className="text-gray-400 mr-2">Quantity:</label>
                        <input type="number" id="quantity" className="w-16 bg-gray-600 text-yellow-500 px-2 py-1 rounded-lg" min={0} value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value))} />
                        <button className="ml-4 bg-yellow-500 text-gray-900 px-3 py-1 rounded-lg" onClick={handleAddToCart}>Add</button>
                    </div>
                )}
            </div>
            <div className="absolute top-2 right-2" ref={dropdownRef}>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="text-red-200">
                    <FaEllipsisH size={20} />
                </button>
                {isDropdownOpen && (
